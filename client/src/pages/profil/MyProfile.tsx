...
export default function MyProfile(): JSX.Element
{
 ...
 /*  function fileChangeHandler(event : React.ChangeEvent<HTMLInputElement> ): void{
  ...
  const deactivate2fa = () => {
    axios
        .post('http://localhost:3001/api/auth/2fa/turn-off', {}, {
    ...
    <>
      {user !== undefined &&
        <div className='profil flex flex-col items-center relative'>
          ...
          <div className='2fa'>
            {enabled ? <button onClick={generate2fa} className="text-slate-200 border-2 border-slate-200"> activate 2fa </button>
            : <button onClick={deactivate2fa}  className="text-slate-200 border-2 border-slate-200"> deactivate 2fa </button>}
            <p>
                <img src={qrcode} />
            </p>
            <form onSubmit={turnOn2fa}>
                <ReactCodeInput type='text' fields={6} onChange={getCode} name={''} inputMode={'url'} />
